...
import mask2former_video.data_video.sn_detection_utils as utils
...
def _get_dummy_anno(num_classes):
    return {
        ...
        "hand_mask": [np.array([0.0] * 6)],
        "contact_mask": [np.array([0.0] * 6)],
...
class YTVISDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        for frame_idx in selected_idx:
            ...
            if not instances.has("hand_masks"):
                instances.hand_masks = BitMasks(torch.empty((0, *image_shape)))

            if not instances.has("contact_masks"):
                instances.contact_masks = BitMasks(torch.empty((0, *image_shape)))
